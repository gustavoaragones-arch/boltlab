...
Baseline: `ca9e59924afa4a2dcd39aade3a3ebc85512a3a51` (T12, committed and pushed)
...
| File | Reason |
|---|---|
| `audit/t13-residual-integrity.md` | T13 discovery narrative + (after review) implementation record |
| `audit/t13-residual-integrity.json` | T13 structured data, discovery + implementation |
| `audit/t13-change-scope.md` | This file |
...
| File | Change |
|---|---|
| `scripts/validators/validate-tapping-projections.js` | Added check 10, "Tap-Drill Status Correctly Derived From Source Cross-Verification" — re-derives each of the 29 profiles' expected `tap_drill.status` directly from its source dataset record's `hole_preparation.cross_verified` (present AND `match !== false` → verified, else source_bound) and fails if it disagrees with the projection's actual value. |
| `docs/architecture/tapping-projection-validation-report.json` | Regenerated validator output reflecting the new check (legitimate T13 output) |
| `docs/architecture/tapping-projection-validation-report.md` | Same, markdown form |

### Files explicitly NOT modified

- `scripts/generators/generate-tapping-projections.js` — deliberately left untouched per the explicit governance constraint: current output is already correct; T13 closes the missing regression guard, not an opportunity to rewrite working projection logic.
...
- `data/datasets/metric_tapping.seed.json` — temporarily mutated during the mandatory mutation test (Test 2), then restored from a pre-mutation backup and confirmed byte-identical via SHA-256 (`d98712267bd52f49bad35cbf1436c230f2eec1921334515e72d959dc94b6a7df`).
...
### Timestamp-only regenerated reports reverted (not part of T13's real diff)
...
None. Pre-existing untracked D2-phase/`.DS_Store`/`.claude`/`images/logo.ai` files predate T13 and were left untouched throughout, matching every prior phase's convention.

## Git status at end of T13
...
?? audit/t13-change-scope.md
?? audit/t13-residual-integrity.json
?? audit/t13-residual-integrity.md
?? css/.DS_Store
?? images/.DS_Store
...
HEAD unchanged throughout: `ca9e59924afa4a2dcd39aade3a3ebc85512a3a51`, matching `origin/main`. Nothing committed or pushed.
