...
## 18. T2 Addendum — Data Acquisition & Verification

T2 (2026-08-15) closed part of the Section 11 gap and refined two design points T1 flagged as open questions. This section documents what changed; it does not restate T1's own record above, which remains historically accurate as of T1.

**Standards acquired.** `iso_2306` ("Drills for use prior to tapping screw threads", 1972, confirmed current/reviewed 2022) and `iso_2857` ("Ground thread taps for ISO metric threads... manufacturing tolerances on the threaded portion", 1973 + 2 amendments, confirmed current/reviewed 2022) were added to `data/standards/iso/standards.seed.json`, using only publicly available scope/title metadata independently verified via ISO's own catalogue records — no copyrighted standard text reproduced. `asme_b94_9` ("Taps: Ground Thread with Cut Thread Appendix", 2008 edition reaffirmed 2023) was added to `data/standards/asme/standards.seed.json`, directly evidencing the physical characteristics already recorded (T1, Section 5) for `spiral_flute_tap`, `spiral_point_tap`, and `forming_tap`. `taper_tap`, `plug_tap`, `bottoming_tap`, and `hand_tap` were **not** related to `asme_b94_9`, since the source material reviewed in this phase named only the fluting/point/forming styles explicitly — chamfer-length styles were not independently confirmed against this standard and no relationship was assumed.

**Standards corrected.** `iso_965_1` and `asme_b1_1` were updated, not replaced — both had public, near-primary confirmation of an edition-level change: ISO 965-1:2013 is withdrawn and ISO 965-1:2026 (5th edition, released 2026-04-14) is current; ASME B1.1's current (2024) edition title and scope explicitly cover UN, UNR, *and* UNJ thread forms (UNJ was absorbed from the former B1.15 as of the 2019 edition), whereas BoltLab's existing record title named only "UN and UNR". Both records gained new optional `edition`/`standard_status` fields (`data/schemas/standard.schema.json`, additive) so this kind of edition tracking doesn't again silently go stale.

**Standards still not acquired.** No standard was cross-verified against its own published data tables (drill-size tables, tolerance tables). Knowing a standard exists, is current, and is topically relevant is not the same as having verified BoltLab's numeric values against its content — see the derived_candidates note below.

**Hole-preparation axis clarification (T1's first caution).** T1 correctly avoided turning the 5 `hole_preparation` entities into a single mutually-exclusive filter, but left the axis distinction implicit. T2 made it explicit with the smallest possible change: an optional `taxonomy_axis` field on `entity.schema.json` (`hole_geometry` | `hole_function` | `preparation_stage`), applied only to the 5 existing hole_preparation entities — `through_hole`/`blind_hole` → `hole_geometry`; `tapped_hole`/`clearance_hole` → `hole_function`; `pilot_hole` → `preparation_stage`. No entity definition, relationship, or dataset record was altered. `tapped_hole` genuinely spans both a function (accepts a mating thread) and an end-state (tapping is complete) — this field records only its primary axis; the nuance is captured here in prose rather than by adding a second schema field for a single edge case.

**RELATES_TO sufficiency (T1's second caution).** Re-evaluated against T2's actual new data (3 new standard↔entity connections, expressed via the pre-existing `related_entities` array already used by every standard record — not via `relationships.seed.json` at all). No new relationship record was added in T2 and no new predicate was needed. The question of whether `RELATES_TO`'s 41 existing edges need finer semantics remains open and is deferred again — T2's own acquisitions didn't require resolving it.

**Derived-candidate pattern (tap-drill/engagement math).** Two verified, real formulas were found — ISO 2306's approximate rule (`drill_diameter ≈ nominal_diameter − pitch`) and the general thread-engagement percentage formula (`% = (Dmajor − Ddrill) / (1.29904 × pitch) × 100`, commonly attributed to Machinery's Handbook) — but neither was used to populate or override any `hole_preparation` or `thread_engagement` value on any existing record. Per governance, a mathematical derivation is not a verified value. Both are recorded as `derived_candidates` (new optional array field, `dataset.schema.json`, additive) on `metric_tapping`/`unc_tapping`/`unf_tapping` — structurally separate from `records`, each carrying `status: "derived_candidate"` and `verified: false`, so no generator or future phase can mistake them for verified engineering data. The commonly-cited "75% engagement" target for UNC/UNF tap-drill charts was explicitly **not** written into any record — it appears only in a derived_candidate's descriptive text as an unconfirmed (Tier 3) third-party characterization, not a BoltLab-verified figure.

**Tap-type taxonomy.** T1's 7 tap_type definitions were checked against ASME B94.9's own style breakdown; `spiral_flute_tap`, `spiral_point_tap`, and `forming_tap` matched the standard's named styles directly (see `asme_b94_9.related_entities` above). No definition text was changed — the check confirmed T1's existing structural descriptions rather than requiring correction.

**Tap-drill and thread-engagement numeric acquisition — scope actually completed.** T2 did not attempt a full per-size cross-verification of the 20-record target seed set against a Tier 1 source table in this pass; see `audit/t2-tapping-data-verification.md` for the precise accounting of what was and wasn't acquired. No fabricated value was introduced at any point.
